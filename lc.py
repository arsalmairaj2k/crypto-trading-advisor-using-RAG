...
class CryptoTradingGuideRAG:
    ...
    def fetch_market_data(self, symbols: List[str] = ["bitcoin", "ethereum", "cardano"]) -> List[Document]:
        ...
        for symbol in symbols:
            if symbol in data:
                ...
                rsi = self.calculate_rsi_estimate(market_data)
                ...
                Technical Analysis:
                Estimated RSI: {rsi:.1f} ({'Oversold' if rsi < 30 else 'Overbought' if rsi > 70 else 'Neutral'})
    ...
    def calculate_rsi_estimate(self, market_data: Dict) -> float:
        """Calculate estimated RSI based on available price changes"""
        # Use available price change data to estimate RSI
        changes = []
        for period in ['1d', '7d', '14d', '30d']:
            key = f'price_change_percentage_{period}'
            if key in market_data:
                changes.append(market_data[key])
        
        if not changes:
            return 50.0  # Neutral RSI if no data
        
        # Simple RSI estimation based on recent changes
        avg_change = sum(changes) / len(changes)
        if avg_change > 10:
            return min(80, 50 + (avg_change * 2))
        elif avg_change < -10:
            return max(20, 50 + (avg_change * 2))
        else:
            return 50 + avg_change
    ...
    def update_all_data(self):
        ...
        if all_new_docs:
            ...
            raise ValueError("No new documents fetched. Check API connections.")
    ...
    def get_portfolio_analysis(self, portfolio: Dict[str, float]) -> str:
        ...
        return self.get_trading_advice(question)
